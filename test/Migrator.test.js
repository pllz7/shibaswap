...
describe("Migrator", function () {
  ...
  beforeEach(async function () {
    ...
    this.topDog = await this.TopDog.deploy(this.bone.address, this.boneLocker.address, this.devBoneDistributorRef.address, this.tBoneBoneDistributorRef.address, this.xShibBoneDistributorRef.address, this.xLeashBoneDistributorRef.address, "1000", "0", "100000")

    await this.topDog.deployed()

    this.migrator = await this.Migrator.deploy(this.topDog.address, this.factory1.address, this.factory2.address, "0")
    ...
    await this.topDog.add("100", this.lp1.address, true)
  })
  ...
  it("should do the migration successfully", async function () {
    ...
    await this.lp1.sync()
    await this.lp1.connect(this.minter).approve(this.topDog.address, "100000000000", { from: this.minter.address })
    await this.topDog.connect(this.minter).deposit("0", "2000000", { from: this.minter.address })
    expect(await this.lp1.balanceOf(this.topDog.address), "2000000")
    await expect(this.topDog.migrate(0)).to.be.revertedWith("migrate: no migrator")

    await this.topDog.setMigrator(this.migrator.address)
    await expect(this.topDog.migrate(0)).to.be.revertedWith("migrate: bad")

    await this.factory2.setMigrator(this.migrator.address)
    await this.topDog.migrate(0)
    expect(await this.lp1.balanceOf(this.topDog.address)).to.equal("0")
    expect(await this.lp2.balanceOf(this.topDog.address)).to.equal("2000000")

    await this.topDog.connect(this.minter).withdraw("0", "2000000", { from: this.minter.address })
